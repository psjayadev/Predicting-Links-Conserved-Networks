...
if (data_flag ==1)
    ...
    [  Inc_Con,Cc_Con,Ab,n_c,e_c,Sptree_branch,Sptree_chord,b,c] = Network_Generation(n,network_flag);
    ...
    [X,Sigma_e] = Data_Generation(e_c,Sptree_branch,Sptree_chord,b,c,Cc_Con,NSamples,SNR,noise_flag,Nrepeats);    
    
end


%% Getting a Linear model from data and finding incidence matrix
if noise_flag == 0
    tstart = tic;
    [Ahat,sin_vals] = Linear_Model(X,e_c,NSamples,noise_flag,Sigma_e); % Finding a linear model by applying PCA on data
    Cc = inv(Ahat(:,Sptree_branch))*Ahat(:,Sptree_chord);
    Cf = [round(Cc) eye(b)];
    Ar = Ab*Cf;
    ...
    time = toc(tstart);
    temp = [[Sptree_chord Sptree_branch];Inc_pred];
